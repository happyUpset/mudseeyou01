...
export const useQuery = ({popstateBeforeWork = (e: Function)=>{}, popstateAfterWork = (e: Function)=>{}} = {}) => {
  ...
  const queriesAsRefs = toRefs(query)
  ...
  const popstateBefore = (res: Function) => {
    if(popstateBeforeWork)
      popstateBeforeWork(res)
  }
  const popstateAfter = (res: Function) => {
  ...
  watch(()=>$route.query,(new1,old1)=>{
    if(queryUpdate.value){
      ...
      queryUseURL.value = true
      popstateBefore(new1)
      updateQuery({
        queries: defaultQuery.value,
        callback: () => {
          queryUseURL.value = false
          queryUpdate.value = false
          popstateAfter(new1)
        }
      })
