...
import {findTimeWithId} from "~/const/duration";
import {computed, ref} from "vue";
import {storeWeb} from "~/store/web";

export const storeForm = defineStore('form', ()=>{
  ...
  const formBase = reactive({
    ...
    tableNumber: [],
    duration: 1,
    // startTime: findTimeWithId(DateTime.now().plus({minute: 1}).toFormat('HH:mm')).id ?? 15,
  ...
  const isBookingTypeOffer = computed(()=> auth.hasOffers && !!options && options.offers && form.value.type === '3')
  ...
  const type3BeyondTheDate = computed(()=>{
    if(isBookingTypeOffer.value && !!options && options.offers && options.offers.length > 0){
  ...
  function set_form_Qrcode(value){
    formQRCode.value = value;
  }
